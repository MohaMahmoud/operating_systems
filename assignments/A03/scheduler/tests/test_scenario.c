...
int main() {
    test_start("You schedule our provided test scenario the right way.");
    initScheduler();
    ...
    // Now process 0 is running.
    for (int i = 0; i < 4; i++) {
        onProcessPreempted(0);
        test_scheduleNextProcess(0);
    }
    onProcessPreempted(0);
    // Now we have scheduled process 0 five times.
    // This means that a lower priority process should be scheduled next.
    test_scheduleNextProcess(1);
