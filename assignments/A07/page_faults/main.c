...
int main()
{
    ...
    // we want retranslation of the same address, so flush the TLB;
    // if we kept the old TLB, an assertion would trigger
    // if we modify the pagetable, the TLB needs to be flushed anyway (see later)
    flushTLB();
    ...
    swapOut(0x10000);
    swapOut(0x11000);
    flushTLB(); // a lot of page table modifications happened, so the TLB needs to be flushed
    ...
    flushTLB(); // retranslation follows, so the TLB needs to be flushed
    test_equals_int64(_doAddressConversion(0x10000, ACCESS_READ, USER_MODE), 0x620000, "Read should work");
    test_equals_int64(_doAddressConversion(0x11000, ACCESS_READ, USER_MODE), 0x621000, "Read should work");
