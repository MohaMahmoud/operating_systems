...
/*
 * This trampoline is used to ensure stack alignment in our user-level threads.
 * Insert this function in the initial stack of new threads to ensure that all
 * "real" functions have a 16 byte alignment.
 *
 * This function is "naked", which means that it can only contain inline
 * assembly code and the compiler will not generate any additional code (such
 * as a function prologue or epilogue).
 */
__attribute__((naked))
static void _trampoline(void) {
    // retq will pop an address from the stack and jump to it.
    __asm__ __volatile__ ("retq" ::: "memory");
}
...
int startThread(void (*func)(void))
{
    ...
    for (int i = 0; i < MAX_THREADS; ++i) {
        if (_threads[i].state == STATE_UNUSED) {
            ...
            void **stackTop = (void**)((char*)stackBase + STACK_SIZE);
            ...
            // stackTop is initially aligned to a multiple of 16 bytes.
            assert(((uintptr_t) stackTop & 0xF) == 0);
            // Use _trampoline to ensure stack alignment.
            (void) _trampoline;
